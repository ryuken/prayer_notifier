...
$(document).ready(function () {
	...
	$.ajax({
   	...
   	{
		$("#date").html(times.Day + "-" + times.Month + "-" + times.Year);
	...
	try {
		...
		socket.onmessage = function got_packet(msg) {
			...
			case "queue":
				...
				if ("undefined" !== typeof obj.data[obj.data.length - 1] && obj.data[obj.data.length - 1].pos + 1 >= pagination + MAX_ELEMENTS_PER_PAGE)
			...
			case "browse":
				...
				for (var item in obj.data) {
					...
					case "directory":
						...
						break;
					case "playlist":
						if (-1 !== ["azan.playlist", "temp.playlist"].indexOf(obj.data[item].plist)) {
							break;
						}
						$('#salamisandwich > tbody').append(
						...
						break;
					case "song":
						if (-1 !== ["azan12.mp3", "bleep.mp3"].indexOf(obj.data[item].uri)) {
							break;
						}
